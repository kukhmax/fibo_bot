...
class TestSignalPipelineResilience(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_app_blocks_symbol_not_in_whitelist(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            original_sleep = asyncio.sleep
            config = load_environment_config("dev")
            config = dc_replace(config, ml=dc_replace(config.ml, enabled=False))
            store = TelegramUserProfileStore(cache=StateCache(Path(tmp) / "profiles.json"))
            signal_profile = store.get_or_create(2001, config)
            paper_profile = store.get_or_create(2002, config)
            store.save(dc_replace(signal_profile, mode="signal_only"))
            store.save(dc_replace(paper_profile, mode="paper", max_open_positions=3, open_positions_count=0))
            transport = _FakeTransport()
            ...
            with (
                patch.dict(
                    "os.environ",
                    {
                        "ENABLE_SIGNALS": "1",
                        "FIB_SYMBOL": "BTCUSDT",
                        "FIB_STRATEGY": "auto_regime",
                        "WHITELIST_SYMBOLS": "ETHUSDT,SOLUSDT",
                    },
                ),
                patch("core.bot.main._run_runtime", _runtime_stub),
                patch("core.bot.main.asyncio.sleep", _fast_sleep),
            ...
            messages = [text for (_chat_id, text) in transport.sent_messages]
            self.assertTrue(any("asset_filter=symbol_not_allowed:BTCUSDT" in item for item in messages))
            updated_paper = store.get(2002)
            assert updated_paper is not None
            self.assertEqual(updated_paper.open_positions_count, 0)

    async def test_run_app_blocks_entry_on_high_news_risk(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            original_sleep = asyncio.sleep
            config = load_environment_config("dev")
            config = dc_replace(config, ml=dc_replace(config.ml, enabled=False))
            store = TelegramUserProfileStore(cache=StateCache(Path(tmp) / "profiles.json"))
            signal_profile = store.get_or_create(3001, config)
            paper_profile = store.get_or_create(3002, config)
            store.save(dc_replace(signal_profile, mode="signal_only"))
            store.save(dc_replace(paper_profile, mode="paper", max_open_positions=3, open_positions_count=0))
            transport = _FakeTransport()
            ...
            with (
                patch.dict(
                    "os.environ",
                    {
                        ...
                        "NEWS_HEADLINE": "Exchange hack causes liquidation risk",
                    },
                ),
                patch("core.bot.main._run_runtime", _runtime_stub),
                patch("core.bot.main.asyncio.sleep", _fast_sleep),
            ...
            messages = [text for (_chat_id, text) in transport.sent_messages]
            self.assertTrue(any("risk_blocked: news_filter=" in item for item in messages))
            updated_paper = store.get(3002)
            assert updated_paper is not None
            self.assertEqual(updated_paper.open_positions_count, 0)
