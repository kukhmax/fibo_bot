...
Дата/время (UTC): 2026-03-07
Подэтап: `E7.13`
Что сделано: Реализован UI ML-отчета и кнопка запуска обучения.
Какие файлы изменены: `core/bot/commands.py`, `core/bot/reporter.py`, `tests/test_bot_router.py`, `DEVELOP.md`
Реализованная логика: 
1. Обновлен `MlQualityReporter`: теперь он показывает понятное сообщение, если модель не обучена.
2. Добавлена кнопка "🚀 Обучить модель" в ML-отчете.
3. Реализован `ml_train_handler`: запускает `MlTrainingPipeline` на 3000 свечах BTCUSDT 5m (baseline).
4. Обновлены тесты `test_bot_router.py` для проверки ML-отчета и процесса обучения.
Команды: `python -m unittest tests.test_bot_router -v`
Тесты: Тесты ML-отчета и хендлера обучения успешно пройдены.
Как проверено: Ручная проверка `/ml_report` -> кнопка "Обучить" -> получение отчета об успешном обучении.
Результат: ML-функционал доступен пользователю через UI.
Commit: `99554f8`
...
Дата/время (UTC): 2026-03-07
Подэтап: `E7.14`
Что сделано: Исправлен UI обучения ML-модели (Spinner, Instant Feedback).
Какие файлы изменены: `core/bot/runtime.py`, `core/bot/telegram_transport.py`, `core/bot/commands.py`, `DEVELOP.md`
Реализованная логика: 
1. Добавлена поддержка `callback_query` в `IncomingMessage` и `TelegramTransportProtocol`.
2. Реализован метод `answer_callback_query` для остановки спиннера на кнопке.
3. `TelegramBotRuntime` теперь автоматически подтверждает получение callback-запроса.
4. `ml_train_handler` теперь возвращает обновленный ML-отчет сразу после обучения, чтобы пользователь видел результат без повторного запроса.
Команды: `docker compose up -d --build`
Тесты: Ручная проверка нажатия на кнопку "Обучить модель" -> спиннер пропадает, отчет обновляется.
Как проверено: Локальный запуск скрипта репродукции `repro_ml_train.py` подтвердил работоспособность пайплайна.
Результат: UX ML-обучения стал плавным и понятным.
Commit: `9fd6651`
...
Дата/время (UTC): 2026-03-07
Подэтап: `E7.15`
Что сделано: Реализован функционал Запуска/Остановки бота и отслеживание активных пар.
Какие файлы изменены: `core/bot/profile.py`, `core/bot/commands.py`, `core/bot/main.py`, `tests/test_bot_router.py`, `DEVELOP.md`
Реализованная логика: 
1. В `TelegramUserProfile` добавлен флаг `is_running` (по умолчанию `False`).
2. В Главное меню добавлена кнопка "▶️ Старт" / "⏹ Стоп" (команда `/run on/off`).
3. Команда `/start` теперь показывает текущий статус (ЗАПУЩЕН/ОСТАНОВЛЕН).
4. `core/bot/main.py` теперь учитывает флаг `is_running`:
   - `_collect_runtime_pairs` возвращает пары только запущенных пользователей.
   - `on_candle` обрабатывает сигналы только для запущенных пользователей.
   - Если пользователь не запустил бота, пайплайны для его пар останавливаются (экономия ресурсов).
5. Обновлены тесты: добавлен тест на переключение `/run` и проверку флага в профиле.
Команды: `python -m unittest tests.test_bot_router -v`
Тесты: Тесты `test_bot_router.py` успешно пройдены.
Как проверено: Ручной тест переключения Старт/Стоп -> изменение статуса в меню -> проверка логов (pipeline start/stop).
Результат: Пользователь имеет полный контроль над запуском торгового/сигнального движка.
Commit: `6413969`
...
Дата/время (UTC): 2026-03-07
Подэтап: `E7.16`
Что сделано: Добавлено подробное логирование торговых операций и вебсокетов.
Какие файлы изменены: `core/bot/main.py`, `core/data/websocket_client.py`, `core/data/pipeline.py`, `DEVELOP.md`
Реализованная логика: 
1. `logging` внедрен в ключевые модули вместо `print`.
2. Логирование WebSocket: подключение, подписки, разрывы, ошибки, размер бэкфилла.
3. Логирование Pipeline: процессинг свечей, проблемы качества данных (Data Quality issues).
4. Логирование Торгового ядра (`main.py`): 
   - Генерация сигналов (стратегия, режим, уверенность).
   - Блокировка сигналов (Risk, News, ML, Whitelist, DD, MaxPos) - для отладки "ложных" сигналов.
   - Отправка сигналов пользователю.
Команды: `docker compose logs -f app`
Тесты: Ручная проверка логов при запуске бота и генерации сигналов.
Как проверено: Запуск бота, наблюдение за логами `ws_connecting`, `pipeline_started`, `signal_generated`.
Результат: Полная прозрачность работы бота через логи.
Commit: `a659457`
...
Дата/время (UTC): 2026-03-07
Подэтап: `E7.17`
Что сделано: Исправлен баг, когда кнопка "Старт" вызывала ошибку после изменения настроек (maxpos, risk и т.д.).
Какие файлы изменены: `core/bot/commands.py`
Реализованная логика: 
1. Добавлен вызов `_clear_flow(ctx.user_id)` во все setter-хендлеры (`set_risk`, `set_rr`, `set_dd`, `set_maxpos`, `set_sl`, `set_tp`).
2. Это гарантирует выход бота из состояния "ожидания ввода" после успешного обновления настройки.
3. Ранее бот оставался в состоянии `await_maxpos` (или другом), из-за чего последующие команды (например, "▶️ Старт") интерпретировались как неверный ввод числа.
Команды: `docker compose up -d --build`
Тесты: Ручная проверка - установить maxpos=5, нажать Старт -> ошибки нет.
Результат: Критический UX-баг исправлен.
